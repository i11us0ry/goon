package brute

import (
	"encoding/binary"
	"encoding/hex"
	"context"
	"fmt"
	"goon3/public"
	"net"
	"runtime"
	"strings"
	"time"
)

var (
	negotiateProtocolRequest, _  = hex.DecodeString("")
	sessionSetupRequest, _       = hex.DecodeString("")
	treeConnectRequest, _        = hex.DecodeString("00000060ff534d4275000000001807c00000000000000000000000000000fffe0008400004ff006000080001003500005c005c003100390032002e003100360038002e003100370035002e003100320038005c00490050004300240000003f3f3f3f3f00")
	transNamedPipeRequest, _     = hex.DecodeString("0000004aff534d42250000000018012800000000000000000000000000088ea3010852981000000000ffffffff0000000000000000000000004a0000004a0002002300000007005c504950455c00")
	trans2SessionSetupRequest, _ = hex.DecodeString("0000004eff534d4232000000001807c00000000000000000000000000008fffe000841000f0c0000000100000000000000a6d9a40000000c00420000004e0001000e000d0000000000000000000000000000")
)

func MS17010(ips []string) {
	runtime.GOMAXPROCS(runtime.NumCPU())
	input := make(chan string,  len(ips))
	result := make(chan string, len(ips))
	defer close(input)
	/* 添加扫描数据 */
	go func() {
		for _,ip := range(ips){
			input <- ip
		}
	}()
	thread := 10
	if len(ips) < Par.Thread {
		thread = len(ips)
	} else {
		thread = Par.Thread
	}
	/* 开启扫描 */
	for i := 0; i< thread; i++{
		go ms17010Work(input, result)
	}
	/* 输出 */
	public.Out(result, Par.Ofile)
}

func ms17010Work(input chan string,result chan string){
	for {
		task,ok := <-input
		if !ok{
			return
		}
		ctx, cancel := context.WithCancel(context.Background())
		scan := make(chan string)
		go MS17010Scan(ctx,task,scan)
		select {
		case flag,ok:= <-scan:
			if flag == "" || !ok{
				result<-""
			} else if flag != "" {
				/* 如果返回pass，说明密码错误，继续爆破 */
				result <- flag
			}
		case <- time.After(time.Duration(Par.Timeout) * time.Second):
			result<-""
			cancel()
		}
	}
}

func MS17010Scan(ctx context.Context, ips string, scan chan string) {
	// connecting to a host in LAN if reachable should be very quick
	select {
	case <-ctx.Done():
		return
	default:
		Host1 := strings.SplitN(ips,":",-1)
		ip := Host1[0]
		conn, err := net.DialTimeout("tcp", ip+":445", time.Duration(Par.Timeout)*time.Second)
		defer func() {
			if conn != nil{
				conn.Close()
			}
		}()
		if err != nil {
			//fmt.Printf("failed to connect to %s\n", ip)
			scan <- ""
			return
		}
		err = conn.SetDeadline(time.Now().Add(time.Duration(Par.Timeout) * time.Second))
		if err != nil {
			//fmt.Printf("failed to connect to %s\n", ip)
			scan <- ""
			return
		}
		_, err = conn.Write(negotiateProtocolRequest)
		if err != nil {
			scan <- ""
			return
		}
		reply := make([]byte, 1024)
		// let alone half packet
		if n, err := conn.Read(reply); err != nil || n < 36 {
			scan <- ""
			return
		}

		if binary.LittleEndian.Uint32(reply[9:13]) != 0 {
			// status != 0
			scan <- ""
			return
		}

		_, err = conn.Write(sessionSetupRequest)
		if err != nil {
			scan <- ""
			return
		}
		n, err := conn.Read(reply)
		if err != nil || n < 36 {
			scan <- ""
			return
		}

		if binary.LittleEndian.Uint32(reply[9:13]) != 0 {
			// status != 0
			//fmt.Printf("can't determine whether %s is vulnerable or not\n", ip)
			scan <- ""
			return
		}

		// extract OS info
		var os string
		sessionSetupResponse := reply[36:n]
		if wordCount := sessionSetupResponse[0]; wordCount != 0 {
			// find byte count
			byteCount := binary.LittleEndian.Uint16(sessionSetupResponse[7:9])
			if n != int(byteCount)+45 {
				fmt.Println("[-]", ip+":445", "ms17010 invalid session setup AndX response")
			} else {
				// two continous null bytes indicates end of a unicode string
				for i := 10; i < len(sessionSetupResponse)-1; i++ {
					if sessionSetupResponse[i] == 0 && sessionSetupResponse[i+1] == 0 {
						os = string(sessionSetupResponse[10:i])
						os = strings.Replace(os, string([]byte{0x00}), "", -1)
						break
					}
				}
			}

		}
		userID := reply[32:34]
		treeConnectRequest[32] = userID[0]
		treeConnectRequest[33] = userID[1]
		// TODO change the ip in tree path though it doesn't matter
		_, err = conn.Write(treeConnectRequest)
		if err != nil {
			scan <- ""
			return
		}
		if n, err := conn.Read(reply); err != nil || n < 36 {
			scan <- ""
			return
		}

		treeID := reply[28:30]
		transNamedPipeRequest[28] = treeID[0]
		transNamedPipeRequest[29] = treeID[1]
		transNamedPipeRequest[32] = userID[0]
		transNamedPipeRequest[33] = userID[1]

		_, err = conn.Write(transNamedPipeRequest)
		if err != nil {
			scan <- ""
			return
		}
		if n, err := conn.Read(reply); err != nil || n < 36 {
			scan <- ""
			return
		}

		if reply[9] == 0x05 && reply[10] == 0x02 && reply[11] == 0x00 && reply[12] == 0xc0 {
			//fmt.Printf("%s\tMS17-010\t(%s)\n", ip, os)
			//if runtime.GOOS=="windows" {fmt.Printf("%s\tMS17-010\t(%s)\n", ip, os)
			//} else{fmt.Printf("\033[33m%s\tMS17-010\t(%s)\033[0m\n", ip, os)}
			result := fmt.Sprintf("[MS17010] %s\tMS17-010\t(%s)", ip, os)
			public.Success.Println(result)
			// detect present of DOUBLEPULSAR SMB implant
			trans2SessionSetupRequest[28] = treeID[0]
			trans2SessionSetupRequest[29] = treeID[1]
			trans2SessionSetupRequest[32] = userID[0]
			trans2SessionSetupRequest[33] = userID[1]

			_, err = conn.Write(trans2SessionSetupRequest)
			if err != nil {
				scan <- ""
				return
			}
			if n, err := conn.Read(reply); err != nil || n < 36 {
				scan <- ""
				return
			}

			if reply[34] == 0x51 {
				result := fmt.Sprintf("[MS17010] %s has DOUBLEPULSAR SMB IMPLANT", ip)
				scan <- result
			}

		} else {
			result := fmt.Sprintf("[*] %s  (%s)", ip, os)
			scan <- result
		}
		scan <- ""
	}
}
