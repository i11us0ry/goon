...
func CheckPar(){
	...
	*/
	public.ConfValue.Web = public.InputValue.WebPtr
	public.ConfValue.NoPing = public.InputValue.NoPingPtr
	// thread和timeout
	...
	if public.InputValue.OfilePtr !=""{
		public.ConfValue.Ofile = public.InputValue.OfilePtr
	}
	/*
			...
			if  public.InputValue.ModePtr == "title" || public.InputValue.ModePtr == "dfuzz" || public.InputValue.ModePtr == "finger" || public.InputValue.ModePtr == "tomcat"{
				for _,line := range(lines){
					if strings.Contains(line, "http"){
						...
						public.Error.Println(fmt.Sprintf("url %s is err!",line))
					}
				}
