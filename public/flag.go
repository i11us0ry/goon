...
func Flag() {
	...
	flag.BoolVar(&InputValue.PingPtr, "ping", false, "ping，不进行icmp探活, 而是调用ping来探活，适用于非root/admin权限用户")
	flag.BoolVar(&InputValue.NoOutputPtr, "no", false, "no，noOutput，不输出扫描日志，避免linux系统下文件报错")
